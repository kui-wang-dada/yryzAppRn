...
export default class SvgUri extends Component {
	constructor(props) {
		super(props);
	...
	createSVGElement(node, childs) {
		...
		case 'svg':
			...
			componentAtts.width = this.props.width || this.width;
			componentAtts.height = this.props.height || this.height;
	...
	render() {
		...
		let flattenStyle = this.props.style;
		if (typeof flattenStyle === 'number') {
			flattenStyle = StyleSheet.flatten([flattenStyle]);
		}

		this.width = flattenStyle.width;
		this.height = flattenStyle.height;
		let svgXmlData = this.state.svgXmlData || this.props.svgXmlData;
		try {
			if (!svgXmlData) {
				return null;
			}
