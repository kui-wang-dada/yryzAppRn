...
export default class Slider extends PureComponent {
	...
	componentDidMount() {
		// alert('image: ' + this.props.thumbImage);
		// Image.getSize(this.props.thumbImage, (width, height) => {
		// 	this.setState({ width, height });

		// 	let state = this.state;
		// 	state.thumbSize = { width: width, height: height }
		// 	this.setState({
		// 		state
		// 	})
	...
	render() {
		...
		var thumbLeft = value.interpolate({
			...
			outputRange: [-thumbSize.width / 2, containerSize.width - thumbSize.width / 2],
			//extrapolate: 'clamp',
		});
	...
	_handleMeasure = (name: string, x: Object) => {
		...
		if (name === 'thumbSize' && this.props.showThumbnail === false) {
			this[storeName] = { width: 0, height: height };
		} else {
			this[storeName] = size;
		}
